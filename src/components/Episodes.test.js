...
test('Render episodes', () => {
    ...
    let episodeArr = screen.queryAllByTestId(/episode/i)
    expect(episodeArr).toHaveLength(0)

    rerender(<Episodes episodes={episodeData} />)
    episodeArr = screen.getAllByTestId(/episode/i)
    expect(episodeArr).toHaveLength(4)
})
